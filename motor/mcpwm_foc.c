...
//				curr1 = -(curr0 + curr2);
	...
	if (motor_now->m_state == MC_STATE_RUNNING) {
		...
		//两次采样平均，V0/V7 或特殊双采样两个采样点的电流平均一下，可以更接近这个 PWM 周期的中心电流，减小采样时刻偏差对电流环的影响
...
static void control_current(motor_all_state_t *motor, float dt) {
	...
	// vd = Rs*id   +   Ld*did/dt 鈭?  蠅e*iq*Lq
	// vq = Rs*iq   +   Lq*diq/dt +  蠅e*id*Ld     + 蠅e*蠄m
	float dec_vd = 0.0;
	float dec_vq = 0.0;
	float dec_bemf = 0.0;
	...
	if (motor->m_control_mode < CONTROL_MODE_HANDBRAKE && conf_now->foc_cc_decoupling != FOC_CC_DECOUPLING_DISABLED) {
		...
		case FOC_CC_DECOUPLING_CROSS:
			dec_vd = state_m->iq * motor->m_speed_est_fast * motor->p_lq; // m_speed_est_fast is 蠅e in [rad/s]
